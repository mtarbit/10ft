  ...
  if (urls.length) {
    ...
    if (id) {
      ...
      feed.fetchVideo(id, function(videoData){
        ...
        var video = feed.buildVideo(videoData);
        video.save(function(err){
            if (err) return console.log(err) && false;

            var tweet = feed.buildTweet(tweetData, video._id);
...
feed.buildTweet = function(data, videoId) {
  var user = data.user
...
feed.buildVideo = function(data) {
  var group = data.entry.media$group;

  var video = new Video({
      id: group.yt$videoid.$t
    , thumb: group.media$thumbnail[1].url
